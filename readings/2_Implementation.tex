...
This is computed using the Fermat's little theorem by the
exponentiation to $2^{255}-21$ with the Square-and-multiply algorithm and taking
...
\begin{coq}
Theorem CSM_eq : forall n p,
  length n = 16 ->
  length p = 16 ->
  Forall (fun x => 0 <= x < 2^8) p ->
  Forall (fun x => 0 <= x < 2^8) n ->
  CSM n p = ZofList n $\times$ ZofList n.
Qed.
\end{coq}

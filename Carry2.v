...
| _,  a,[]     => [a]
| 0%nat,  a,h::q   => (a + h) :: q
| S p,a,h :: q => getResidute n (a + h + 2^n) :: Carrying_n p (getCarry n (a + h + 2^n) - 1) q
end.

Lemma Carry_n_step: forall m a h q, Carrying_n (S m) a (h :: q) = getResidute n (a + h + 2^n) :: Carrying_n m (getCarry n (a + h + 2^n) - 1) q.
...
Proof.
  ...
  - assert(Hpos: 2^n > 0) by (apply pown0 ; auto);
    simpl ; flatten ; go;
    rewrite! ZofList_cons ;
    rewrite <- IHm ;
    unfold getResidute;
    rewrite Zplus_mod ;
    replace (2 ^ n mod 2 ^ n) with 0 by (symmetry; apply Z_mod_same_full) ;
    rewrite <- Zred_factor6;
    rewrite Zmod_mod ;
    unfold getCarry ;
    rewrite Z.shiftr_div_pow2 by omega;
    try replace (0 + z + 2 ^ n) with (0 + z + 1 * 2 ^ n) by omega ;
    try replace (Z.pos p + z + 2 ^ n) with (Z.pos p + z + 1 * 2 ^ n) by omega ;
    try replace (Z.neg p + z + 2 ^ n) with (Z.neg p + z + 1 * 2 ^ n) by omega ;
    rewrite Z.div_add by omega;
    try replace ((0 + z) / 2 ^ n + 1 - 1 + (ℤ.lst l0)) with ((0 + z) / 2 ^ n + (ℤ.lst l0)) by omega;
    try replace ((Z.pos p + z) / 2 ^ n + 1 - 1 + (ℤ.lst l0)) with ((Z.pos p + z) / 2 ^ n + (ℤ.lst l0)) by omega;
    try replace ((Z.neg p + z) / 2 ^ n + 1 - 1 + (ℤ.lst l0)) with ((Z.neg p + z) / 2 ^ n + (ℤ.lst l0)) by omega;
...
Definition Zcar25519 (n:ℤ) : ℤ  :=  38 * getCarry 256 n +  getResidute 256 n.

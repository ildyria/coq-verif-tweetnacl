...
Local Ltac solve_this_assert :=
  ...
  rewrite sub_step_2_Z_inv; Grind_add_Z; try assumption;
  rewrite ?sub_fn_rev_s_sub_step_2_Zlength ; try omega;
  apply bound_a_subst_step_2 ; auto ; omega.
...
assert(Hbound: forall a, 0 < a /\ a < 16 -> - 2^16 < nth (Z.to_nat (a - 1)) m 0 <= 0).
{
  ...
  apply Forall_nth_d.
  compute ; split ; [reflexivity | discriminate].
  assumption.
  apply nth_take_full.
...
assert(H2: ℤ16.lst sub_fn_rev_s 1 sub_step_2 2 m = ℤ16.lst m).
  rewrite sub_fn_rev_s_n ; try apply sub_step_2_Z_inv; [omega | | omega].

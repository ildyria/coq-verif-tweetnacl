...
Local Lemma sub_fn_rev_s_sub_step_1_ind_Zlength : forall a m t,
  0 <= a < Zlength m ->
  Zlength (sub_fn_rev sub_step_1 a m t) = Zlength m ->
  Zlength (sub_fn_rev sub_step_1 (a+1) m t) = Zlength m.
Proof.
...
Local Lemma sub_fn_rev_s_sub_step_1_ind_bound : forall a m t,
  ...
  Forall (fun x => -2^16 + 1 <= x < 1) (take (Z.to_nat a) (sub_fn_rev sub_step_1 a m t)) ->
  Forall (fun x => -2^16 + 1 <= x < 1) (take (Z.to_nat (a + 1)) (sub_fn_rev sub_step_1 (a + 1) m t)).
Proof.
...
Proof.
intros.
change(Forall (fun x => -2^16 + 1 <= x < 1) (take (Z.to_nat i) (sub_fn_rev sub_step_1 i m t))) with
  ((fun x => Forall (fun a => -2^16 + 1 <= a < 1) (take (Z.to_nat x) (sub_fn_rev sub_step_1 x m t))) i).

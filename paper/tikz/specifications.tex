\begin{tikzpicture}[textstyle/.style={black, anchor= south west, align=center}]
    ...
    \filldraw[draw=orange!10!doc@lstbackground, fill=doc@lstbackground, thick] (0.25,0.5) rectangle (6.25,6.5);
    % node[textstyle, anchor=west, draw=yellow, fill=yellow!20, thick, minimum width=5.5cm,minimum height=5cm] {};

    \draw (6.25,6.5)  node[anchor=north east, inner sep=0pt] (russell) {\includegraphics[width=.03\textwidth]{img/coq_logo.png}};
    ...
    \draw [thick, dotted, ->] (longlong.north) -- (clight.south);
    ...
    \begin{scope}[yshift=1 cm,xshift=0 cm]
      ...
      \draw (0.5,4) node [textstyle, anchor=west, draw=black, thick, minimum width=3cm,minimum height=0.5cm] (hl) {$\Ffield$};
      \draw (0,4) node [anchor=east] (skn) {High Level};
      % \draw (0,4) node [anchor=east] (shn) {Mid Level};

      \draw[thick,double, <->, >=implies] (ml.north) -- (hl.south);

      \draw (0.5,5) node [textstyle, anchor=west, draw=black, thick, minimum width=3cm,minimum height=0.5cm] (kl) {$\K$};
      \draw (0,5) node [anchor=east] (skn) {ECC theory};

      \draw[thick,double, <-, >=implies] (hl.north) -- (kl.south);

      \draw (4.5, 2.5) node  [textstyle, anchor=west, draw=black, thick, minimum width=1.5cm,minimum height=0.5cm] (rfcl) {\coqe{RFC}};

      \draw[thick,double, <->, >=implies] (ml.east) -- (4.5,2.6);
      \draw[thick,double, <->, >=implies] (ll.east) -- (4.5,2.4);

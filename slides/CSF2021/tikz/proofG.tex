\begin{tikzpicture}[textstyle/.style={black, anchor= south west, align=center, minimum height=0.45cm, text centered, font=\scriptsize}]
	...
	% SECTION III
	\draw (-1cm,-0.75cm) node[anchor=east] {\includegraphics[width=2cm]{MontgomeryG.png}};
	\draw (-3.25cm,-0.75cm) node[textstyle,anchor=east] {\texttt{tweetnacl.c}};
	\draw (-0.5cm,-0.75cm) node[] {$\implies$};

	\draw (-1cm,-3.5cm) node[anchor=east] {\includegraphics[width=2cm]{rfcG.png}};
	\draw (-3.25cm,-3.5cm) node[textstyle,anchor=east] {RFC~7748};
	\draw (-0.5cm,-3.5cm) node[] {$\implies$};

	\draw (-1cm,-7.75cm) node[anchor=south east] {\includegraphics[width=2cm]{curvePaperG.png}};
	\draw (-3.25cm,-7cm) node[textstyle,anchor=east] {Maths};
	\draw (-0.5cm,-7cm) node[] {$\implies$};
	...
	\begin{scope}[yshift=-6.5 cm,xshift=0 cm]
		\draw (0,0) -- (0.4, 0.4) -- (1.75,0.4) -- (1.75,0) -- cycle;
		% \draw[fill=yellow!20] (0,0) -- (1.75,0) -- (1.75,-1) -- (0, -1) -- cycle;
		\draw (0,0) -- (1.75,0) -- (1.75,-1) -- (0, -1) -- cycle;
		\draw (0.3,-0.05) node[textstyle] {Definition};
	...
	\begin{scope}[yshift=-6 cm,xshift=6 cm]
		...
		\draw (0.3,-0.05) node[textstyle] {Proof};
		\draw (1.25,-0.75) node[textstyle, anchor=center] {{\color{doc@lstnumbers}\textbf{Pre}} $\Rightarrow$\\$\text{\texttt{RFC}}(n,P) = n \cdot P$};
		\draw (2.5,0) node[textstyle, anchor = south east] {\color{doc@lstfunctions}{\checkmark}};
